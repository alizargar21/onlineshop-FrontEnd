...
import SelectSort from "../../common/SelectComponent/SelectSort";
import Select from "../../common/SelectComponent/Select";
...
const SearchSort = ({ originalValue, setRenderValue, renderValue }) => {
  ...
  return (
    <section className=" 2xl:sticky 2xl:top-[90px] md:static text-[12px] md:w-[90%] md:h-[150px] w-[230px] h-[450px] md:mx-auto  m-5 rounded-lg p-5 bg-gray-300 dark:bg-black/30">
      ...
      <div className="w-[80%] mx-auto">
        <Select items={brands} onChange={selectBrandHandler} />
      </div>
      <div className="mx-auto w-[80%]">
        <SelectSort options={sortPriceOptions} onChange={(sortHandler)} />
      </div>
    </section>
  );
};
